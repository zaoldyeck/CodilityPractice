...
def solution(a: Array[Int], b: Array[Int]): Int = {
  ...
  val c: Array[BigDecimal] = a zip b map {
    case (int1, int2) => int1 + BigDecimal(int2) / 1000000
  }

  var count = 0
  for (i <- 0 to c.length - 2) {
    for (ii <- i + 1 until c.length) {
      if (c(i) * c(ii) >= c(i) + c(ii)) count += 1
    }
  }

  if (count > 1000000000) 1000000000 else count
}
solution(Array(0, 1, 2, 2, 3, 5), Array(500000, 500000, 0, 0, 0, 200000)) // should return 8
solution(Array(0, 1, 2, 2, 3, 5), Array(500000, 500000, 0, 0, 0, 200000))